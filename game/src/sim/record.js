...
export function makeRecord(sim, input, bus, opts, perf) {
  ...
  const rec = {
    ...
    player: {
      ...
      anim_len: p.animLen,
      phase: p.phase,
      // ---- W1-10 / WEAPON-CRITIC §3.1's four required frame-record fields -------------------
      anim_slot: p.animSlot === undefined ? null : p.animSlot,
      hitstop_f: p.hitstopF === undefined ? 0 : p.hitstopF,
      hitstop_held: !!p.hitstopHeld,
      weapon_tip: p.weaponTip || null,
      weapon_guard: p.weaponGuard || null,
      // The state RI-WPN04 M3 / RI-WPN06 M1 read rather than infer.
      weapon_id: p.weaponId === undefined ? null : p.weaponId,
      weapon_class: p.weaponClass === undefined ? null : p.weaponClass,
      stance: p.stance === undefined ? null : p.stance,
      offhand_kind: p.offhandKind === undefined ? null : p.offhandKind,
      roll_tier: p.rollTier === undefined ? null : p.rollTier,
      charge_f: p.chargeF === undefined ? 0 : p.chargeF,
      charge_max_f: p.chargeMaxF === undefined ? 0 : p.chargeMaxF,
      chains_to: p.chainsTo === undefined ? null : p.chainsTo,
      block_angle_deg: p.blockAngleDeg === undefined ? null : p.blockAngleDeg,
      block_success_f: p.blockSuccessF === undefined ? null : p.blockSuccessF,
      guard_raised: !!p.guardRaised,
