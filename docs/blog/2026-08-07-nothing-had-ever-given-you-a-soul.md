...
it worked and none of it could be paid for. A builder who was looking for something else entirely
spawned an ambush, killed five of the six men in it, and watched the counter go from zero to zero
with the next level still costing 418.
...
number in that sum is tuned so an ordinary marsh sentry pays 136 souls, which is what the design
document says an average early kill should be worth. Everything else falls out: a champion is
worth 2,423, and the training dummy with 99,999 hit points is worth nothing at all, which is what
stops it being a machine that prints levels.
...
:::compare The same screen, at the same campfire, a few hours apart. On the left the character is
holding 4,200 souls because a test wrote the number in. On the right they are holding 816, which
is what six men in a raid party were worth.
...
Every soul value on disk is anchored to an old estimate of how many enemies the finished game
would contain — 576 — which the design has since replaced with about 1,230. There is a written
rule for exactly that situation and it was not applied: at the newer count, each enemy should be
worth a little under half what it is now. The marsh sentry should pay 64. Clear the whole world at
today's values and you arrive at level 120 to the point, and the same document says in terms that
level 120 is not, and must not become, where a first playthrough ends.

The critic also killed something with an actual sword rather than a test command — forty-two
swings, twenty-four seconds of swinging — because all twelve of the builder's own checks had
killed by decree, and none of them proved that a fight pays. It found, separately, that anything
killed by an environmental hazard pays nothing and then gets back up.
...
against a plan of finishing a first run somewhere around level 82. An hour later a different agent
landed the outdoor population — 144 groups, 267 bodies, 34,646 souls, which is level 24. Indoors
is not attempted, and the file says so at the top rather than quietly counting zero.
