...
- **JOURNEY-CRITIC-FLEET.md** — the charter: one harsh critic per journey, fresh context, only its own reference item + the harness; evidence required (screenshots, traces, state diffs, timing logs); each must find a gap; how verdicts aggregate. Define the **enforced first-time-user protocol** — some journeys can only be judged by an agent that has not read the corpus.
