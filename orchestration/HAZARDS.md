...
On 2026-08-14 the disk hit 96%. The space was in `.git`, which had grown to **9.3 GB**; a
`git gc --prune=now` returned **8.3 GB** and took it to 1.2 GB. That part was right, and it is still
the first thing to try when the box is tight — ahead of deleting anything, because the obvious
culprits were seven ~1.2 GB **live null-control clones**, which are delete-the-fix work in progress.
...
nearly the same space. With a dozen agents holding staged work, `--prune=now` is the only copy of
anything not yet committed.

## 2. `git status` clean is not evidence your work is in the tree

Six agents have now had finished work silently reverted. Two causes, both real:

- **Two agents holding one file.** A whole-file write erases the other's edit regardless of git.
  Ruling O1 in `OWNER-DIRECTIVES-2026-08-14.md` binds the orchestrator to run
  `node tools/ownership.mjs --for <path>` and `--conflicts`, and to check `ListAgents`, before
...
git add -A                      # or: git update-index --add --cacheinfo from a hashed blob
...
For a single heavily-contended file, build the new content in `/tmp`, `git hash-object -w` it, and
`update-index --cacheinfo` it onto the remote tree — the working copy may be reverted underneath you
between your edit and your `git add`. That happened three times to this file's predecessor.
...
whose entire purpose was to be broken — into the repository, and at another point a bank deleted an
agent's report from disk. Exclude sabotage copies, null-control clones and scratch. A periodic sweep
for already-tracked sabotage artefacts is owed; roughly sixteen are in the tree.
...
- The proxy takes **40–90 s to start routing** after a Pod reads `RUNNING`, so a run budget under
  ~4 minutes will not survive it.

## 5. The disk fails silently

Captures were failing with `ENOSPC` and looking exactly like clean runs. `tools/lib/browser.mjs`
now refuses to launch on short free space, covering all 247 harness tools; override deliberately with
