# COST.md — the full text as it stood on 2026-08-14, before the read-path cleanup

**This is the evidence, not the instruction.** The programme as it binds *now* is
`orchestration/COST.md`, which every agent reads on cold start. This file is that document as it stood
at the moment of the cleanup, in full and unedited: every superseded reading, every correction the plan
loop caught, the levers ranked as hypotheses in §4, the external-evidence survey in §4a, and the
complete `docs/data/cost-ledger.json` data contract in §6.1.

**Nothing here was deleted.** It was moved off the mandatory read path, where it was costing every
agent ~17,000 tokens before it did anything. Read it when you want to know why a ruling says what it
says, or before you re-adopt something this programme already killed.

**Section numbers cited elsewhere resolve here.** `tools/cost.mjs` and other files cite
`COST.md §2`, `§4.0`, `§4.1`, `§5` and `§6.1`; the live document was renumbered, and **this archive
keeps the original numbering**, so a citation to `COST.md §6.1` (the `docs/data/cost-ledger.json` data
contract) means §6.1 of the text below.

**One thing in the text below is superseded and is called out here because it is the trap.** G1 was
**inverted** on the evening of 2026-08-14 — from a floor of 12 concurrent agents to *roadmap steps
delivered* — so every sentence below that asserts, assumes or guards a parallelism floor is history.
The live `COST.md` §1 states the guard that binds. In particular Ruling C2's mean-versus-median
argument is about a statistic that is no longer a guard; what survives of C2 is its methodological
core, *never select a metric after seeing which reading passes*.
