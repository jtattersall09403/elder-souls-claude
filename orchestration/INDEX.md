...
so it cannot drift. Generated at `ed4f5fbd`: 927 tools, 155 reference
items, 173 pieces in flight.

Its purpose is to stop 173+ concurrent agents each paying separately to discover the
...
> **245 tools have no header comment**, so nobody can tell what they do without
...
| `W1-30B` | landed-pending-fresh-critic | Fresh critic: (1) run the child-scoped Deck plus the census sample plus the whole open inv | `game/src/render/sky.js` `game/src/render/lighting.js` `game/src/render/lib/lighting-recipes.js` `tools/visual/w1-30b-air.mjs` `tools/visual/w1-30b-probe.mjs` `tools/visual/w1-30b-roughness.mjs` |
...
| `W1-30K` | built-92-rows-set-12-unset-validator-green | FRESH CRITIC. (1) Re-derive 20 numbers: every row names its plates and its statistic, `node docs/art-direction/measure-plates.mjs` regenerates the layer under them. (2) Audit the 12 unset rows for convenience rather than honesty — especially `ART-X7-SEPARATION-RAW`, unset because Morrowind's own regions fail its test at LOO 9.1% against 11.1% chance. (3) Attack the three cross-cutting judgements (`ART-X1`, `ART-X2`, `ART-X6`) on whether the statistic carries the look, not on the arithmetic. The current-build column is empty and the reason is on every row. | `docs/art-direction/board.json` `docs/art-direction/ART.md` `docs/art-direction/FIDELITY.md` `docs/art-direction/README.md` `docs/art-direction/measure-plates.mjs` `docs/art-direction/build-board.mjs` `docs/art-direction/emit-docs.mjs` `docs/art-direction/validate-board.mjs` `docs/art-direction/plate-metrics.json` `orchestration/status/W1-30K.json` |
