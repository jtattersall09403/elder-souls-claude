...
- `tools/harness/critic-t4-journal.mjs` — T4 critic: RI-UIX04.
- `tools/harness/critic-t4-lean.mjs` — T4 critic.
- `tools/harness/critic-t4-live2.mjs` — T4 critic, the three live measurements the first fixture run lost: the dialogue window (RI-UIX08 look), the pause rule (RI-UIX03 §A) and the compass withdrawal 
- `tools/harness/critic-t4-probe.mjs` — T4 critic (independent).
- `tools/harness/critic-t4-r1-finish.mjs` — T4 critic, round 1, the measurements the two earlier live runs lost to a renderer crash and a 90 s step timeout: the dialogue window's LOOK (RI-UIX08 §A/§E1), t
- `tools/harness/critic-t4-r1-withdrawal.mjs` — the ONE check that decides AR-2 on this piece, and nothing else.
- `tools/harness/critic-t4-run.mjs` — T4 critic, the whole live pass in one browser session.
- `tools/harness/critic-t4-shots.mjs` — T4 critic.
- `tools/harness/critic-t4-shots2.mjs` — T4 critic, pass 2: the screens the first pass could not reach (dialogue, level-up, book, a journal with something in it) plus the pause-rule and withdrawal meas
...
- `tools/i2/eval-subject.mjs` — Evaluate candidate subject-presence statistics against a REAL labelled set: the F10 hardware run, where the 17 frames of npc-lilmoth-apothecary-12 are known to 
...
- `tools/ui/critic-t4-crop.mjs` — **no header comment**
- `tools/ui/critic-t4-palette.mjs` — RI-UIX06 A4/AD4, run by hand because the item's named invocation (`palette-conformance.mjs --in <dir> --palette regions.json`) is not the CLI that tool has: it 
...
- `tools/ui/dialogue-drive-probe.mjs` — RI-UIX08's dialogue window, OPERATED.
- `tools/ui/dialogue-input-diag.mjs` — where does a real key press die on its way to the dialogue window?
...
- `tools/ui/t4-r2-critic-drive.mjs` — THE MORROWIND SCREENS, OPERATED.
- `tools/ui/t4-r2-critic-final.mjs` — the three things left, and the pictures.
- `tools/ui/t4-r2-critic-focus.mjs` — the five things `t4-r2-critic-drive.mjs` could not settle, plus the RI-UIX07 withdrawal measurement that neither the round-1 critic nor the round-2 builder got.
- `tools/ui/t4-r2-critic-ju5.mjs` — RI-UIX04 JU5, the append-only byte-diff.
- `tools/ui/t4-r2-critic-measure.mjs` — RI-UIX09 §A, measured independently of the builder's instrument.
- `tools/ui/t4-r2-critic-twohand.mjs` — is `two_hand` (the HUD full/minimal switch) dead, or is my clock?
- `tools/ui/t4-r2-evidence.mjs` — writes `corpus/90-verdicts/wave1/artifacts/T4-r2/EVIDENCE.md` from the measurement JSONs, so every number in that document is read off a file at the moment it i
- `tools/ui/t4-r2-measure.mjs` — re-runs the T4 round-1 critic's own measurements so round 2 has comparable figures, and adds the two RI-UIX09 rows the self-report cannot answer.
- `tools/ui/t4-r2-withdrawal.mjs` — RI-UIX07 E-T2/E-T3/V7/V8 — the withdrawal, on a fixture that HAS SOMETHING TO FIGHT.
...
- `tools/visual/character-digit-read.mjs` — **no header comment**
- `tools/visual/character-face-relief.mjs` — **no header comment**
- `tools/visual/character-refs.mjs` — build (and check) the character reference set routing table.
- `tools/visual/character-texel-density.mjs` — **no header comment**
...
- `tools/visual/f10-character-sweep.mjs` — **no header comment**
- `tools/visual/f10-r2-appearance.mjs` — **no header comment**
- `tools/visual/f10-r2-pair-sheet.mjs` — **no header comment**
- `tools/visual/f10-r3-materials.mjs` — **no header comment**
- `tools/visual/f10-r4-digits.mjs` — **no header comment**
- `tools/visual/f10-silhouette.mjs` — **no header comment**
- `tools/visual/frame-liveness.mjs` — **no header comment**
...
- `tools/visual/mesh-winding.mjs` — **no header comment**
...
- `tools/visual/w1-f3-ambient-fill.mjs` — **no header comment**
...
> **296 of 1048 tools have no header comment**, so nobody can tell what
