...
| `F1` | **Materials and surface response** | Delivered, then **judged FAIL 1/10** — the fix exposed a duplicate-uniform collision that stopped **66 of 124** actor-body programs linking, so no character had a body. **Regression fixed and confirmed: 124/124 link**, verified at 8 orbit angles both arms. Outstanding: delete-the-fix unconfirmed (the gate hardcodes its entry and silently ignores `--entry` — a real tool bug), and the new `runnable` census tripwire has never been run |
