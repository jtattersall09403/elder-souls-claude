...
| `F7` | The water surface | **Delivered `b0f3224e`, critic in flight — and it is a partial fix its own builder refused to call finished.** Two premises the piece started from were false and had to go first: the lanes are **not axis-aligned** (world bearing **−50°**, measured top-down; `water.js`'s header describes an *already-fixed* defect and has been read as present tense since), and rounds 1–2's **nine negative results are unreadable**, because their crop's dominant structure is a *terrain* weave that survives hiding every water mesh. The owner is **`esReflection`**: `esGrazing` took `max(esFresnel, smoothstep(10,52,dist)*.72)` and the distance term carried **no view-angle factor**, so it *overrode* Fresnel — every water pixel past 52 m composited a **0.610** reflection weight at any viewing angle, including straight down, where real water reflects ~2%. Multiplying by `(1.0-esView)` moves only the wrong case (down 0.610 → 0.250; grazing and close-oblique unchanged). **It removes 28% of the streaks, not the streaks** (rms 7.711 → 5.484) and the residual is plainly visible — *no downstream doc may call the marsh lanes fixed.* Named and untouched: the reflection target is half-resolution (`renderer.js:125`) and refreshed every 6 frames (`renderer.js:1424`). |
