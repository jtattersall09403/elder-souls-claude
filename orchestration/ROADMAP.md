...
| | | state |
|---|---|---|
| `G1` | **The camera** — the instrument the body is judged through | `RI-CAM01` 2/10. **§C.1 is now MEASURED against the shipped build (15 Aug) and the automatic fail is NOT cleared:** longest run of `player_occluded` frames **≥ 16** against a bar of **≤ 6**, dense every-frame sampling, `armLen` pinned at 3.993 m with `armHit` false throughout. **The vegetation diagnosis and its fix (`b476b55d`) are correct but orthogonal** — both arms of the delete-the-fix are byte-identical here because the occluder is **thin deck architecture in Lilmoth**, not a canopy: `updateOcclusion` only ever hides `kind === 'canopy'` instances, and a 0.28 m sphere cast passes between deck posts. This is §C.1's own named case and M2b's `cam-walk-boardwalk` route. **Next piece is thin-architecture occlusion, not more vegetation work.** Also open: `RI-CAM01`'s blind pair is unrun (rule 0e, orchestrator-only); `RI-CAM03` unjudged (needs ~25,200 locked frames — a round of its own, and the `onScreen(eid)` harness method it requires does not exist); **`RI-CAM04` is not judgeable at all** — `roll_bin` is not traced and 8 of its 9 directional roll clips do not exist, so that is corpus/build debt, not a zero |
...
| `F3` | **Ambient fill** | **Delivered.** +0.631 luma lift on the shadow floor, replicated to three decimals across three GPUs, control −0.001. Judged with F1/F2 in Protocol A r2 and did not move it: `gi_off` shifts the judged window **1.51** against a **1.25** noise floor. Real, correct, and small. |
| `F4` | **Light, sky and atmosphere** | **IN FLIGHT, and it is the unlock for the whole ring.** Measured light budget: sun **19.1%**, sky IBL probe **39.9%**, hemisphere+ambient 8.0%, **all indirect 45.7%** — and the indirect cannot be occluded, because the probe is an analytic dome `sky.js` itself says holds *"no terrain, no settlement and no canopy"*, and `aoMap` is null on **53 of 53** on-screen materials. **Turning off the entire 4096² shadow map moves the judged pixels 0.78, below the 1.25 noise floor; the sky dome moves them 21.79.** Acceptance: `sun_off` delta ≥ 2× `env_off` delta in all five judged windows — today it is the wrong way round by 2.1×. |
| `F5` | The frame pipeline — post, tonemap, exposure | |
| `F6` | Terrain and vegetation | |
| `F7` | The water surface | **Delivered `b0f3224e`, judged `W1-F7-WATER-r1` — FAIL at 2, not done.** The build's diagnosis is real and re-derives exactly: `esGrazing` took `max(esFresnel, smoothstep(10,52,dist)*.72)`, the distance term had no view-angle factor, and every water pixel past 52 m carried a **0.610** reflection weight at any angle. **But the fix stops at the constant floor under the term it moved** — `water.js:111` still composites a fixed **0.25 × 0.68 = 17%** of a half-res, up-to-6-frame-stale reflection into every water pixel at normal incidence, where water reflects ~2%. The critic's three findings: the headline **−28.9%** is measured on an instrument that rises **+42.8%** when the water is replaced by a flat colour, and it is mostly the water getting **16.9% darker** (water-masked, brightness-normalised streaking falls **27%**, not 29%, and fine-scale contrast rises 5.8%); the same tool's **`lane_power` rose 12.1%** and **`lane_aniso` rose 48.1%** and were not reported; and **`RI-VIS03` M12 `FresnelDelta` is negative on both arms (−0.044 → −0.078)**, moving away from its ≥ 0.05 band — the corpus's own instrument for exactly this property, never run by the build. **The four poses a player occupies are indistinguishable between the arms (+0.8%, −0.3%, −0.4%); only straight-down-at-120 m moves (−21.3%).** `RI-WLD10`'s M56 blind gate has still never run — the orchestrator owes it (rule 0e). Next: `GAP-W1-F7-water-is-a-25-percent-mirror-at-normal-incidence`. |
