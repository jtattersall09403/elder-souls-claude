...
| Term | Definition |
|---|---|
| **Traversal minute (TM)** | one minute of movement along the critical path at reference walk speed **3.4 m·s⁻¹** (204 m per TM), measured with zero combat and zero pauses |
...
| # | Rule | Value |
|---|---|---|
| AM1 | Ambush density | ≤ 1 per 1.5 TM, and ≤ 3 per bonfire loop |
...
| Metric | Target | Hard range |
|---|---|---|
| Enemy instances per TM | **2.8** | 1.8 – 4.2 |
| Encounters per TM | **1.5** | 1.0 – 2.2 |
| Enemies per encounter (mean) | 1.9 | 1.4 – 2.6 |
| Traversal minutes per bonfire loop | **5.0** | 3.5 – 7.0 |
| Enemies per bonfire loop | **14** | 9 – 22 |
| Encounters per bonfire loop | **8** | 6 – 12 |
| Solo : grouped encounter ratio | **55 : 45** | 45:55 – 65:35 |
| Gank duos per loop (RI-AI05 A10) | 1 | 0 – 2 |
| Ambushes per loop | 1–2 | 0 – 3 |
| ELITE encounters per loop | 0 or 1 | ≤ 1 |
| Fraction of encounters evadable without combat | **0.35** | 0.25 – 0.50 |
| Longest stretch with zero enemies | 0.6 TM | ≤ 1.0 TM |
| Longest stretch with zero *rest* (no enemy-free 15 s) | — | ≤ 1.2 TM |

Region modifiers applied to "enemy instances per TM" (RI-AI05 §D regions):

| Region | Multiplier | Effective enemies/TM |
|---|---|---|
| R1 | 0.85 | 2.4 |
| R2 | 1.00 | 2.8 |
| R3 | 1.05 | 2.9 |
| R4 | 1.10 | 3.1 |
| R5 | 1.00 | 2.8 (density falls, lethality rises) |
...
Rules: B3 must never be immediately adjacent to the terminal (≥ 0.4 TM of B4 between them,
...
Region R3 (refHP 950). Critical path 1 040 m ⇒ **5.1 TM**. Terminal: Vakka-Zhil fog gate
...
|---|---|---|---|
| Traversal minutes | 5.1 | 3.5 – 7.0 | ✓ |
| Enemy instances | 14 | 9 – 22 | ✓ |
| Enemies per TM | 2.75 | 1.8 – 4.2 (R3 target 2.9) | ✓ |
| Encounters | 8 | 6 – 12 | ✓ |
| Encounters per TM | 1.57 | 1.0 – 2.2 | ✓ |
...
| Longest enemy-free stretch | 0.55 TM (E8 → fog gate) | ≤ 1.0 TM | ✓ |
| B1/B2/B3/B4 TM fractions | 0.20 / 0.36 / 0.26 / 0.18 | within all bands | ✓ |
| B3 → terminal separation | 0.44 TM | ≥ 0.4 TM | ✓ |
...
For each bonfire loop: `TM = path_length_m / 204`. Count enemy instances and encounters
...
- **PASS** if every metric is inside §D's hard range and enemies/TM is within ±0.6 of the
  region target.
- **Headline number: enemy instances per traversal minute; fail if outside 1.8 – 4.2.**
...
10. **Density inflation as difficulty.** R4 is R2 with twice as many enemies per metre.
    Enemies/TM climbs past 5, encounters merge, and the level becomes attrition. §D's hard
...
`provenance: constructed`. The traversal-minute unit, the 204 m/TM reference, every density
