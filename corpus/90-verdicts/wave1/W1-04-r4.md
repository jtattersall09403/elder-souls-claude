...
* **The uncapped 115-room live distinctness sweep.** The dispatch asked whether an offline count of
  a scene graph is evidence about what a player sees, and told me to run the 115 live if it is not.
  I ran **32 doors live** for the §1 attribution and **40** for §5, and the answer for those 72
  door-visits is that the live measure behaves exactly as the offline one does — but **the 115 is
  still offline-only**, and my own first attempt at it died the same way the builder's did. The
  honest position is the builder's own: *"round 2's '92 distinct signatures over 115' is not
  re-taken at 115 live."* It still is not. What has changed is that we now know the instrument is
  sound (§2), which is the part that was actually in doubt.
* **Whether a live signature equals the offline signature for the same room.** The builder said
  plainly that it did not verify this and that the two traversals start from different roots. My
  L4 section was written to close it and was still running when I filed. Until someone lands it,
  the offline 115 and the live 40 are two numbers wearing one name, and neither this verdict nor
  the survey should be read as claiming otherwise.
* **The formal degradation check** (delete `getDrawnSignature` from the harness and confirm 10a
  goes to FAIL). The code path is unambiguous by reading — `sigStreet` becomes `null`, the count
  requires it, so every door falls out and the check goes DEAD — and I state that as a reading,
