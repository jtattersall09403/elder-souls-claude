...
**Swept over the whole population**, entering each interior from a province start with three steps
and a `renderFrame()` each: **115 of 115 entered** — 0 refused, 0 errors — and **0 of 115 switched
the drawn cell.** The visible set reads `["province"]` after every door in the game.
`reports/critic-w1-04-r1c.json`.
...
* **AR-2 passes, and I measured it rather than asserting it.** Standing in Thorn: map markers 0,
  routes drawn 0, quest-bearing elements none, travel affordances none, places drawn 0; a
  six-element HUD at 2.05% coverage with no minimap and no compass. `getDrawnMarkers()` returns
  exactly one object in the whole town and it is `hearth-thorn`, the Hist-shrine RI-WLD03's service
  matrix requires. `tryPlaceMapMarker` exhausted thirteen routes to put a pin on Thorn and landed
  none; `openMenu('map', {place})` and `openMenu('minimap')` both throw with the S35 refusal text.
  W1-04 adds no marker surface of its own, and its door prompt is a diegetic reach state that
  contributes nothing to the HUD census.
...
* My verdict, its JSON, the three images and my three instruments were swept into another agent's
  commit `92378c9` (a blanket `git add -A`) while I was still writing. **I ran `git commit` at no
  point.** Every number here is stamped to `3ba93e0`, which is an ancestor of HEAD, and I re-ran
  nothing after the sweep. This is the third time the pattern is on record.
