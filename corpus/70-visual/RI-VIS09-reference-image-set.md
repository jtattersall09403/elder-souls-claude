...
| `morrowind/REF-A12b/` | **33** | Steam Community, native engine | **The appearance half, and it has been on disk since 2026-08-06** (see the correction box below §1). Dialogue ×5, journal ×3, inventory ×4, map ×5, character sheet ×4, barter ×3, book reader ×4, tooltip ×2, level-up ×2, char creation ×1. `vanilla_confidence: high`, eye-verified on one contact sheet, `metrics_valid_for: ["art-direction","ui-fidelity"]`, `pixel_metrics_valid: false`. |
| `morrowind/REF-A12c/` | **1** | **the project owner**, OpenMW for Android | The owner's own dialogue capture, 2376×1069 — the highest-resolution interface reference we hold, supplied 2026-08-14. Carries explicit `licence` and `licence_position` fields of its own because its provenance differs from every other file here. **Two known non-Morrowind elements, both named on the record and in `RI-UIX08` §E: the topic list renders green (no vanilla `[FontColor]` entry is that colour) and the corner chrome is the Android touch front-end.** `vanilla_confidence: medium`, `modded: "unresolved"`. |
