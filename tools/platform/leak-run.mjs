#!/usr/bin/env node
// leak-run.mjs — ABSENCE-REPORTER.
//
// Named by: RI-PLT02 (memory and asset budgets).
//
...
//   JS heap and GPU memory across N load/play/unload loops, and whether the resting heap returns to its first-loop level
//
...
const USAGE = `
leak-run.mjs — ABSENCE-REPORTER for: heap/GC access.

USAGE
  node tools/platform/leak-run.mjs --loops 10 --seed 4711 --profile phone-mid --out reports/platform/<runId>

OPTIONS
  --loops N      loops the real instrument would drive
...
const exit = await reportAbsence({
  tool: 'tools/platform/leak-run.mjs',
  items: ["RI-PLT02 (memory and asset budgets)"],
