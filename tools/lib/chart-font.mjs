...
/**
 * DELETE-THE-FIX (rule 6). `CHART_FONT_BROKEN=1` restores the pre-fix 23-character table for the
 * whole process, so any chart tool can be run in its broken arm without editing the tree — which
 * matters on a tree where a dozen agents are staging concurrently and a neighbour's `git add -A`
 * would otherwise stage a temporarily-broken shared module. It shouts on stderr so it can never
 * be on by accident, and `tools/analysis/w1-chartfont-deletefix.mjs` checks the arm actually
 * bit: it requires the broken arm to reproduce the archived pre-fix PNGs byte-for-byte.
 */
const BROKEN = process.env.CHART_FONT_BROKEN === '1';
if (BROKEN) process.stderr.write('chart-font: CHART_FONT_BROKEN=1 — DRAWING WITH THE PRE-FIX SHEARED TABLE\n');

/** The font, as {char: 25-character bit string}. Index it as bits[j * GLYPH_W + i]. */
export const FONT = BROKEN ? SHEARED_FONT : compile(ROWS);
