#!/usr/bin/env node
// audio-budget.mjs — ABSENCE-REPORTER.
//
// Named by: RI-AUD02 (web audio budget).
//
...
//   concurrent voice count, decoded audio bytes resident, and AudioContext node count against RI-AUD02's ceilings, from audio-stats.json and audio-log.json
//
...
const USAGE = `
audio-budget.mjs — ABSENCE-REPORTER for: audio.

USAGE
  node tools/analysis/audio-budget.mjs --run reports/runs/<runId>

OPTIONS
  --run DIR      a run directory carrying audio-stats.json
...
const exit = await reportAbsence({
  tool: 'tools/analysis/audio-budget.mjs',
  items: ["RI-AUD02 (web audio budget)"],
  system: 'audio',
