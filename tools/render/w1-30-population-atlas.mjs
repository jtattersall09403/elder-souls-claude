...
const disabledFeatures=String(a.disableFeatures||'').split(',').map(x=>x.trim()).filter(Boolean),disableGeology=String(a.disableGeology||'false')==='true';
...
try{
 ...
 if(requested.has('regions'))for(const r of regions){await h.h('loadState','default');await h.h('setUIVisible',false);await h.h('setWeather','clear');await h.h('setTimeOfDay',10.5);const focus=regionFocus(r),[x,z]=[focus.x,focus.z],settled=await settle(x,z),geology=disableGeology?await h.h('__w1_30_drawGeology',false):null,t=await h.h('getTerrainAt',x,z),y=t.y;await h.h('camera',{pos:[x-28,y+8.5,z-24],look:[x+13,y+2.8,z+18],fov:61,mode:'free'});await save('regions',r.id,{name:r.name,centroid:r.centroid_m,captureFocus:[+x.toFixed(1),+z.toFixed(1)],centroidOffsetM:focus.radius,terrain:t,water:r.water,canopy:r.props.canopy,under:r.props.under,cover:r.props.cover,settled,geologyControl:geology});}
