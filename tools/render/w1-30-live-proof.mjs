...
try{
  ...
  await shot('vegetation-atmosphere','vegetation/atmosphere',async()=>{await handle.h('loadState','swamp_canopy');await handle.h('setTimeOfDay',9);await handle.h('setWeather','fog');await handle.h('stepFrames',12);return await handle.h('getEnvConditions');});
  ...
  const report={schema:'elder-souls/w1-30-live-proof@1',result:rows.length===9&&rows.every(r=>r.pixels.uniqueColours>32&&r.pixels.range>8&&r.pixels.meanLuma>=lumaFloors[r.id])&&rows[0].observation.mechanismPerturbation.changed&&rows.find(r=>r.id==='spell-impact').observation.impactVfx.systems>=3&&refusal.cleanRefusal&&handle.errors.length===0?'GREEN':'RED',build:handle.buildInfo,browser:handle.browser.version(),chromiumArgs:handle.chromiumArgs,lumaFloors,rows,refusal,pageErrors:handle.errors};
