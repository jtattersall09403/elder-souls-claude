...
try{
 ...
 await shot('exterior_marsh_dusk','exterior_lowlight',async()=>{await h.h('loadState','default');await h.h('setUIVisible',false);await h.h('setTimeOfDay',18.5);await h.h('setWeather','fog');return place(3173,3280,[-18,4.8,13],[24,2.4,-25],61);});
 ...
 await shot('exterior_marsh_noon','exterior_daylight',async()=>{await h.h('setTimeOfDay',12);await h.h('setWeather','clear');return place(3173,3280,[-15,5.2,12],[25,2.8,-26],62);});
 await shot('water_edge','exterior_lowlight',async()=>{await h.h('loadState','default');await h.h('setUIVisible',false);await h.h('setTimeOfDay',6.4);await h.h('setWeather','dawn_mist');return place(3438,2896,[-6,3.1,5],[33,3.6,-38],55);});
 await shot('interior_rootway','interior_darkemissive',async()=>{await h.h('loadState','dungeon_primary');await h.h('setUIVisible',false);await h.h('stepFrames',18);await cam([1.45,1.72,-7.4],[-.45,1.30,9.5],58);return{camera:await h.h('camera',{}),conditions:await h.h('getEnvConditions')};});
 ...
 await shot('xanmeer_vista','exterior_daylight',async()=>{await h.h('loadState','default');await h.h('setUIVisible',false);await h.h('setTimeOfDay',10.5);await h.h('setWeather','clear');return place(3477,2857.5,[-23,8.2,24],[0,5.4,0],58);});
