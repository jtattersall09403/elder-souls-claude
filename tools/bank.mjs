...
// LOAD-AWARE SKIP, added 2026-08-15. `publish` takes ~70-100s on a quiet box and TIMES OUT at 300s
// when the fleet is capturing — measured repeatedly at load 19-29 over 4 cores, costing five minutes
// per bank and landing nothing. It is also REDUNDANT under load: `.github/workflows/publish.yml`
// runs on every push to this branch, so the site is regenerated by CI regardless. Skipping it locally
// when the box is busy loses nothing and buys back the five minutes.
//
// Deliberately NOT skipping `gen-index` — it is fast and it is what stops agents rediscovering the
// same 1,000 tools.
const loadavg = (() => { try { return Number(readFileSync('/proc/loadavg', 'utf8').split(' ')[0]); } catch { return 0; } })();
const CORES = (() => { try { return cpus().length || 1; } catch { return 1; } })();
const BUSY = loadavg > CORES * 2.5;
if (BUSY) console.log(`bank: load ${loadavg.toFixed(1)} over ${CORES} core(s) — skipping local publish; CI republishes on push.`);

for (const [label, cmd] of [['gen-index', ['tools/gen-index.mjs']], ...(BUSY ? [] : [['publish', ['tools/publish.mjs']]])]) {
