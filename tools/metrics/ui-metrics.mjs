...
USAGE
  node tools/metrics/ui-metrics.mjs [--screens inventory,journal,book,levelup,sheet,spells]
                                    [--scales 1280x720,1920x1080] [--dpr 1,2]
                                    [--in <dir>] [--out <dir>] [--json]

  --in   re-read a capture this tool wrote instead of launching a browser

EXIT 0 = FD1-FD7 all pass · 1 = one or more fail · 2 = could not measure
`;
...
const SCREENS = String(args.screens || 'inventory,journal,book,levelup,sheet,spells').split(',');
  ...
  for (const dpr of DPRS) {
    ...
    try {
      ...
      for (const screen of SCREENS) {
        ...
        // A region known to be body prose: the biggest text-bearing element on the screen.
        ...
        const region = textEls.sort((a, b) => b.rect[2] * b.rect[3] - a.rect[2] * a.rect[3])[0];
        ...
        log(`  ${screen} ${w}x${hpx}@${dpr}: buffer ${buf.buffer.join('x')} image ${on.width}x${on.height} stem ${stem.median} px (${stem.samples} samples)`);
      }
    } finally {
      await h.close();
    }
...
out.native = `${out.checks.filter((c) => c.pass).length}/7`;
