...
for (const p of pages) {
  if (!/^Lore:Flora [A-Z]$/.test(p.title)) continue;
  // UESP's Flora pages are one == Heading == per plant
  for (const h of Object.keys(sections(p.text))) {
    if (h !== '_lead' && h.length < 40 && !/^(References|Notes|Gallery|See Also)$/i.test(h)) floraNames.add(h);
  }
...
const crossCheck = [
  ...
  },
  {
    ...
    source_says: 'Lore:An-Xileel — "a political party in Black Marsh formed sometime during the Oblivion Crisis, consisting primarily, if not entirely, of Argonians. They supported Black Marsh\'s independence from the Empire… Many Argonians firmly held the belief that the An-Xileel were the sole reason that Mehrunes Dagon failed to conquer Black Marsh."',
    verdict: 'confirmed',
    correction: 'Confirmed but softer than the registry: the page says "formed sometime during the Oblivion Crisis", not 3E 433 exactly. CF-067\'s conclusion holds — they do not exist in 3E 427 — and the corpus\'s constructed Xul-Aneekh (CF-C003) is therefore correctly motivated. Additional usable detail: they were a political PARTY, pro-independence, spread anti-Imperial propaganda and took prisoners of war, and many Argonians credited them alone with Dagon\'s failure in Black Marsh.',
  ...
  },
  {
    ...
    source_says: 'Lore:Hist Sap — "when consumed by non-Argonian species, the Hist sap had strong hallucinogenic properties that sent them into dangerous bloodlust". The page adds that the Blackwood Company\'s tree was badly treated, which made its sap affect ARGONIANS the same way; that ingesting large quantities is dangerous even for Argonians; and that the Miredancer Sap-Speakers suffer documented sap-poisoning — "gold tongue" (permanent golden mouth pigmentation), "bark scale" (thickening and darkening of surface scales) and unwanted hallucinations. It also distinguishes Amber Plasm, which looks like sap but is Chaotic Creatia leaking through the Hist "like blood from a wound".',
    verdict: 'confirmed',
    correction: 'Confirmed, and three findings the registry does not have. (1) Sap-poisoning in ARGONIANS is canon and has named, progressive, cosmetic-then-systemic symptoms — the corpus\'s constructed five-band sap-taint (CF-C011) turns out to have a real canonical ancestor and should cite it instead of standing alone. (2) The bloodlust is not intrinsic to non-Argonians: a mistreated Hist produces it in Argonians too, which makes it a fact about the tree\'s condition, not about the drinker\'s race. (3) Amber Plasm — Chaotic Creatia leaking through a Hist "like blood from a wound" — is an unclaimed canonical image sitting directly on top of the corpus\'s constructed sapwell-as-open-wound (CF-C009).',
